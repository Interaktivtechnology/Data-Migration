...
function readMongoCollection(fieldList) {
  ...
  MongoClient.connect(url, function(err, db) {
    ...
    coll.find({
      ParentId: null,
      status: 'new'
    }).limit(LIMIT).skip(SKIP).each((err, res) => {
      ...
      if (res != null) {
        if(action.toLowerCase() == "insert") insert(res, fieldList)
        else if (action.toLowerCase() == "update") update(res)
        else if (action.toLowerCase() == "normalise-account") normaliseAccount(res, coll )
      }
    })
  })
}
...
function insert(res, fieldList, conn)
{
  let newObj = {}
  ...
  newObj.RowStatus__c = res.status
  newObj.Airswift_Record_Id__c = res.RefId
  if(res.status == 'existing' || res.status == 'merged'){
    newObj.Id = res.Id
    conn.sobject(sfTable).upsert(newObj, (err, sfres) => {
      if (err) {
        conn.sobject(sfTable).select("Id").where({
          Airswift_Record_Id__c: res.RefId
        }).execute((err, record) => {
          ...
          }, (err, res) => console.log(err ? err : "No error \n"))
        })
      }
    })
  }
    ...
    conn.sobject(sfTable).insert(newObj, (err, sfres) => {
      coll.updateOne({
        ...
        console.log(err ? err : "No error ", sfres)
        normaliseAccount(res, coll)
      })
...
function normaliseAccount(res, coll)
{
  coll.find({ParentId : {$ne : null}}).each((err, res) => {
    if(res != null)
      AccountColl.updateOne({ParentId: res._id}, {ParentId: res.NewId}, (err, result) => {
